...
public class CampfireManagementScreen extends Screen {
	...
	public void render(DrawContext context, int mouseX, int mouseY, float delta) {
		...
		// 1.21 / 1.21.1: drawTexture(Identifier, x, y, u, v, width, height, texW, texH)
		...
		if (isLit) {
			for (int i = 0; i < 3; i++) {
				...
				context.drawTexture(fireTextures[fireFrame[i]],
					fx, fy,
					0f, 0f,
					LIT_TEX_W, LIT_TEX_H,
					LIT_TEX_W, LIT_TEX_H);
	...
	private void renderSlotTooltip(DrawContext context, int slotIndex, CampfireGuiPacket.SlotInfo slot, int mouseX, int mouseY) {
		...
		// withShadowColor() not available in 1.21 / 1.21.1
		lines.add(Text.literal(seconds + "s remaining").styled(s -> s.withColor(TOOLTIP_TIME_COLOR)));
